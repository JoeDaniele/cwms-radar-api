...
class LocationsDaoTest extends DaoTest {
    ...
    void testDeleteLocation() throws Exception {
        ...
        assertThrows(IOException.class,
            () -> locationsDao.getLocation(location.getName(),
                                           UnitSystem.EN.getValue(),
                                           OFFICE_ID)
        );
    }
    ...
    void testRenameLocation() throws Exception {
        Location location = buildTestLocation();
        Location renamedLocation = new Location.Builder(location)
                                               .withName("RENAMED_TEST_LOCATION2")
                                               .build();
        LocationsDaoImpl locationsDao = new LocationsDaoImpl(
                                                    getDslContext(getConnection(),
                                                    OFFICE_ID)
        ...
        try {
            ...
            locationsDao.deleteLocation(renamedLocation.getName(),
