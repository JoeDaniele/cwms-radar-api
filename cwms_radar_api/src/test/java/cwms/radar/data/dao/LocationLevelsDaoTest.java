...
public class LocationLevelsDaoTest extends DaoTest {
    ...
    void testStore() throws Exception {
        ...
        try {
            ...
            LocationsDao locationsDao = new LocationsDaoImpl(
                                                getDslContext(getConnection(),
                                                OFFICE_ID));
            location = buildTestLocation("TEST_LOC");
            locationsDao.storeLocation(location);
            LocationLevelsDao levelsDao = new LocationLevelsDaoImpl(
                                                getDslContext(getConnection(),
                                                OFFICE_ID));
            levelsDao.storeLocationLevel(levelToStore, ZoneId.of("UTC"));
            LocationLevel retrievedLevel = levelsDao.retrieveLocationLevel(
                                                    levelToStore.getLocationId(),
                                                    UnitSystem.EN.getValue(),
                                                    levelToStore.getLevelDate(), "LRL");
            assertNotNull(retrievedLevel);
            assertEquals(levelToStore.getLocationId(), retrievedLevel.getLocationId());
    ...
    void testDeleteLocationLevel() throws Exception {
        LocationLevelsDao levelsDao = new LocationLevelsDaoImpl(
                                            getDslContext(getConnection(), "LRL"));
        ...
        locationsDao.storeLocation(location);
        levelsDao.storeLocationLevel(levelToStore, ZoneId.of("UTC"));
        LocationLevel retrievedLevel = levelsDao.retrieveLocationLevel(levelToStore.getLocationId(),
                                                                       UnitSystem.EN.getValue(),
                                                                       levelToStore.getLevelDate(),
                                                                       OFFICE_ID);
        assertNotNull(retrievedLevel);
        levelsDao.deleteLocationLevel(levelToStore.getLocationId(),
                                      levelToStore.getLevelDate(),
                                      OFFICE_ID,
                                      true);
        assertThrows(IOException.class,
            () -> levelsDao.retrieveLocationLevel(levelToStore.getLocationId(),
                                                  UnitSystem.EN.getValue(),
                                                  levelToStore.getLevelDate(),
                                                  OFFICE_ID)
        );
    ...
    void testUpdate() throws Exception {
        ...
        LocationsDao locationsDao = new LocationsDaoImpl(
                                            getDslContext(getConnection(), OFFICE_ID));
        LocationLevelsDao levelsDao = new LocationLevelsDaoImpl(
                                            getDslContext(getConnection(), OFFICE_ID));
        ...
        try {
            ...
            existingLocationLevel = levelsDao.retrieveLocationLevel(levelToStore.getLocationId(),
                                                                    UnitSystem.EN.getValue(),
                                                                    levelFromBody.getLevelDate(),
                                                                    OFFICE_ID);
            ...
            updatedLocationLevel = new LocationLevel.Builder(updatedLocationLevel)
                                                    .withLevelDate(levelFromBody.getLevelDate())
                                                    .build();
            if (!updatedLocationLevel.getLocationId()
                 .equalsIgnoreCase(existingLocationLevel.getLocationId())) {
                //if name changed then delete location with old name
                levelsDao.renameLocationLevel(levelToStore.getLocationId(), updatedLocationLevel);
            } else {
                levelsDao.storeLocationLevel(updatedLocationLevel,
                                             updatedLocationLevel.getLevelDate().getZone());
            }
            LocationLevel retrievedLevel = levelsDao
                                                .retrieveLocationLevel(
                                                    updatedLocationLevel.getLocationId(),
                                                    UnitSystem.EN.getValue(),
                                                    updatedLocationLevel.getLevelDate(), OFFICE_ID);
            assertNotNull(retrievedLevel);
        } finally {
    ...
    private LocationLevel deserializeLocationLevel(String body,
                                                   String format,
    ...
    String getExampleJSON() {
        return "{ \"seasonal-values\": [ { \"value\": 10, \"offset-minutes\": 0, \"offset-months\": 0 }],"
               ...
               + "\"location-id\": \"TEST_LOC.Elev.Inst.0.Bottom of Inlet\","
               + "\"office-id\": \"LRL\""
               + "}";
    }
    ...
    String getRenamedExampleJSON() {
        return "{ \"seasonal-values\": [ { \"value\": 10, \"offset-minutes\": 0, \"offset-months\": 0 }],"
               ...
               + "\"interval-months\": 1,"
               + "\"interpolate-string\": \"false\","
               + "\"location-id\": \"TEST_LOC6.Elev.Inst.0.Top of Inlet\","
               + "\"office-id\": \"LRL\""
               + "}";
    }
    ...
    private LocationLevel updatedClearedFields(String body,
                                               String format,
                                               LocationLevel existingLocation) throws IOException {
        ObjectMapper om = getObjectMapperForFormat(format);
    ...
    private LocationLevel getUpdatedLocationLevel(LocationLevel existinglocation, LocationLevel updatedLevel) {
        ...
        final String specifiedLevelId = (updatedLevel.getSpecifiedLevelId() == null ? existinglocation.getSpecifiedLevelId() : updatedLevel.getSpecifiedLevelId());
        final String parameterTypeId = (updatedLevel.getParameterTypeId() == null ? existinglocation.getParameterTypeId() : updatedLevel.getParameterTypeId());
        final String parameterId = (updatedLevel.getParameterId() == null ? existinglocation.getParameterId() : updatedLevel.getParameterId());
        Double siParameterUnitsConstantValue = (updatedLevel.getSiParameterUnitsConstantValue() == null ? existinglocation.getSiParameterUnitsConstantValue() : updatedLevel.getSiParameterUnitsConstantValue());
        final String levelUnitsId = (updatedLevel.getLevelUnitsId() == null ? existinglocation.getLevelUnitsId() : updatedLevel.getLevelUnitsId());
        final ZonedDateTime levelDate = (updatedLevel.getLevelDate() == null ? existinglocation.getLevelDate() : updatedLevel.getLevelDate());
        final String levelComment = (updatedLevel.getLevelComment() == null ? existinglocation.getLevelComment() : updatedLevel.getLevelComment());
        final ZonedDateTime intervalOrigin = (updatedLevel.getIntervalOrigin() == null ? existinglocation.getIntervalOrigin() : updatedLevel.getIntervalOrigin());
        ...
        final String interpolateString = (updatedLevel.getInterpolateString() == null ? existinglocation.getInterpolateString() : updatedLevel.getInterpolateString());
        final String durationId = (updatedLevel.getDurationId() == null ? existinglocation.getDurationId() : updatedLevel.getDurationId());
        final BigDecimal attributeValue = (updatedLevel.getAttributeValue() == null ? existinglocation.getAttributeValue() : updatedLevel.getAttributeValue());
        ...
        final String attributeParameterTypeId = (updatedLevel.getAttributeParameterTypeId() == null ? existinglocation.getAttributeParameterTypeId() : updatedLevel.getAttributeParameterTypeId());
        final String attributeParameterId = (updatedLevel.getAttributeParameterId() == null ? existinglocation.getAttributeParameterId() : updatedLevel.getAttributeParameterId());
        final String attributeDurationId = (updatedLevel.getAttributeDurationId() == null ? existinglocation.getAttributeDurationId() : updatedLevel.getAttributeDurationId());
        final String attributeComment = (updatedLevel.getAttributeComment() == null ? existinglocation.getAttributeComment() : updatedLevel.getAttributeComment());
        final String locationId = (updatedLevel.getLocationId() == null ? existinglocation.getLocationId() : updatedLevel.getLocationId());
        final String officeId = (updatedLevel.getOfficeId() == null ? existinglocation.getOfficeId() : updatedLevel.getOfficeId());
        if (existinglocation.getIntervalMonths() != null
            &&
            existinglocation.getIntervalMonths() > 0) {
            intervalMinutes = null;
        } else if (existinglocation.getIntervalMinutes() != null
                 &&
                 existinglocation.getIntervalMinutes() > 0) {
            intervalMonths = null;
        }
        ...
        } else if (existinglocation.getSeasonalTimeSeriesId() != null
                 &&
                 !existinglocation.getSeasonalTimeSeriesId().isEmpty()) {
            siParameterUnitsConstantValue = null;
            seasonalValues = null;
        }
    ...
    private void deleteLevel(LocationLevel level) throws Exception {
        ...
        levelsDao.deleteLocationLevel(level.getLocationId(),
                                      level.getLevelDate(),
                                      level.getOfficeId(),
                                      true);
    }
    ...
    LocationLevel buildExampleLevel(String locationName) throws Exception {
        ...
        LocationLevel retval = new LocationLevel.Builder(
                                            locationName + ".Elev.Inst.0.Bottom of Inlet",
                                            unmarshalledDateTime)
