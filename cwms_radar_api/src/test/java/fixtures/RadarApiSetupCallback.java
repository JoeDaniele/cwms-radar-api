...
public class RadarApiSetupCallback implements BeforeAllCallback,AfterAllCallback {
    ...
    private void loadDefaultData(CwmsDatabaseContainer cwmsDb) throws SQLException {
        ...
        for (String data: defaultList) {
            String []userResource = data.split(":");
            String user = userResource[0];
            ...
            cwmsDb.executeSQL(
                loadResourceAsString(
                    userResource[1]).replace("&user.",cwmsDb.getPdUser()
                    ),
                    user
            );
        }
    ...
    private ArrayList<String> getDefaultList() {
        ...
        InputStream listStream = getClass()
                .getResourceAsStream("/cwms/radar/data/sql/defaultload.txt");
        try (BufferedReader br = new BufferedReader(
                                    new InputStreamReader(listStream)
                                );
            ) {
            String line = null;
