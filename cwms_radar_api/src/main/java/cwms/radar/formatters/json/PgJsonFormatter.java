...
import cwms.radar.api.graph.pg.dto.PgEdgeData;
import cwms.radar.api.graph.pg.dto.PgGraphData;
import cwms.radar.api.graph.pg.dto.PgNodeData;
...
public final class PgJsonFormatter implements OutputFormatter {
    ...
    /**
     * Modifies the generic graph data with names to match desired output.
    ...
    private List<PgEdgeData> formatEdges(List<Edge> edges) {
        ...
        for (Edge edge : edges) {
            ...
            if (edge instanceof StreamEdge) {
                ...
                PgProperties properties = new PgReachEdgeProperties(
                                                reachEdge.getStreamId(),
                                                reachEdge.getId());
                ...
                throw new IllegalArgumentException(
                    "PG-JSON format does not currently support this Edge type");
            }
            retval.add(edgeData);
        }
        return retval;
        ...
        for (Node node : nodes) {
            if (node instanceof BasinConnectivityNode) {
                ...
                PgProperties properties = new PgStreamNodeProperties(
                                                basinConnNode.getStreamId(),
                                                basinConnNode.getStation(),
                                                basinConnNode.getBank());
                retval.add(new PgNodeData(nodeId, labels, properties));
            } else {
                throw new IllegalArgumentException(
                    "PG-JSON format does not currently support this Node type");
            }
        }
        return retval;
    }
    ...
    public String format(CwmsDTO dto) {
        ...
        if (dto instanceof Basin) {
            ...
            throw new FormattingException(dto.getClass()
                                          .getSimpleName()
